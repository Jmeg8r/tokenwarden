...
run_sut() { if OUT="$("$SUT" 2>&1)"; then RC=0; else RC=$?; fi; }
...
new_repo c8
  ...
  git -c protocol.file.allow=always submodule add -q "$WORK/sub" sub 2>/dev/null \
    || { bad "c8 fixture: could not stage a gitlink — the case would prove nothing"; }
  git ls-files -s sub | grep -q '^160000' \
    || bad "c8 fixture: no gitlink in the index — the case would prove nothing"
  run_sut; expect "gitlink submodule alone" PASS
new_repo c9
  ( mkdir -p "$WORK/sub2" && cd "$WORK/sub2" && git init -q . \
    && git config --local user.email t@t.t && git config --local user.name t \
    && echo x > a && git add a && git commit -qm s >/dev/null ) 2>/dev/null
  git -c protocol.file.allow=always submodule add -q "$WORK/sub2" sub 2>/dev/null \
    || { bad "c9 fixture: could not stage a gitlink — this case would degrade to a plain large-file check"; }
  git ls-files -s sub | grep -q '^160000' \
    || bad "c9 fixture: no gitlink in the index — this case would degrade to a plain large-file check"
  big after.bin; git add after.bin
  run_sut; expect "gitlink does not mask a large file staged after it" BLOCK
...
if OUT="$(PATH="$_shim:$PATH" "$SUT" 2>&1)"; then RC=0; else RC=$?; fi
...
if [ "$RC" -eq 1 ] && printf '%s' "$OUT" | grep -qi 'truncated'; then
...
if printf '%s' "$OUT" | grep -q 'checked 1 staged blob'; then
